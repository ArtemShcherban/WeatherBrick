...
class UserDefaultsManager: UserDefaults {
    ...
    func save(_ myWeather: MyWeather) {
        ...
        let data = try encoder.encode(myWeather)
            set(data, forKey: "myWeather")
        } catch {
            print("Unable to encode MyWeather \(error)")
        }
    }
    
    func getLocation() -> CLLocation? {
        if let data = object(forKey: "myWeather") as? Data {
            if let myWeather = try? JSONDecoder().decode(MyWeather.self, from: data) {
